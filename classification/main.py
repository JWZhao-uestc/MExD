...
parser.add_argument(
    #"-f"
    "--model", #feature_exactor_type
    type=str,
    required=True,
    default = 'transmil',
    help="which base feature_exactor you choose",
)
parser.add_argument(
...
def main():
    ...
    if args.loss == 'card_onehot_conditional':
        from card_classification_v3 import Diffusion
    else:
    ...
    try:
        ...
        if args.sample:
            ...
            if config.data.dataset in ['FashionMNIST', 'MNIST', 'CIFAR10', 'CIFAR100', 'IMAGENE100'] or 'WSI' in config.data.dataset:
