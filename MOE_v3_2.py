...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.gate1 = nn.Linear(self.patch_dim_2, self.num_experts, bias=False)
        ...
        self.classifier = nn.ModuleList([nn.Linear(self.patch_dim_2, args.num_classes) for _ in range(self.num_experts)])
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        for expert_idx in range(self.num_experts): # the 0-th expert predict 0, the 1-th expert predict 1. 
            ...
            # s_idx = torch.where(select_weights > 0.55)
            ...
            #print(selected_patches, selected_patches.shape)
            #patch_mask = torch.nn.functional.one_hot(selected_patches, num_classes=self.num_experts).permute(2, 1, 0)
            #idx, top_x = torch.where(patch_mask[expert_idx])
            ...
            #output = self.trans2(current_hidden_states)
            #output = self.milnet[expert_idx](current_hidden_states.contiguous())
        ...
        print('****', patches_feature.shape, represent_patchs[0].shape, represent_patchs[1].shape)
        ...
        return results_dict, joint_losses, None